...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		mw = 600;
		//mw = MIN(MAX(max_textw() + promptw, 100), info[i].width) + 200;
...
#endif
	{
		if (!XGetWindowAttributes(dpy, parentwin, &wa))
			die("could not get embedding window attributes: 0x%lx",
			    parentwin);
		mw = 600;
		//mw = MIN(MAX(max_textw() + promptw, 100), wa.width) + 200;
		x = (wa.width  - mw) / 2;
		y = (wa.height - mh) / 2;
	}
...
usage(void)
{
	fputs("usage: dmenu [-bfirvS] [-l lines] [-h height]  [-p prompt] [-fn font] [-m monitor]\n"
